...
const createCanvas = (p_id, width=100, height=100) => {
    ...
    setInterval(() => { 
        ret.Draw(); 
        if (ret.showSystem) {
            ret.g.setStrokeStyle("red");
            ret.g.setFillStyle("red");
            ret.g.fillLine(0,ret.center.y,ret.width,ret.center.y);
            ret.g.fillLine(ret.center.x,0,ret.center.x,ret.height);

            let scale = 50;
            for (let i = 0; i < 100; i++) {
                ret.g.fillLine(ret.center.x+i*scale,ret.center.y-2,ret.center.x+i*scale,ret.center.y+2); 
                ret.g.fillText(i*scale,ret.center.x+i*scale,ret.center.y);

                ret.g.fillLine(ret.center.x-i*scale,ret.center.y-2,ret.center.x-i*scale,ret.center.y+2);
                ret.g.fillText(-i*scale,ret.center.x-i*scale,ret.center.y);
                
                ret.g.fillLine(ret.center.x-2,ret.center.y-i*scale,ret.center.x+2,ret.center.y-i*scale);
                ret.g.fillText(-i*scale,ret.center.x,ret.center.y-i*scale);
