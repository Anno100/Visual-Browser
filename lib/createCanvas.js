...
const createCanvas = (width = 100, height = 100, p_id = 'canvas_' + canvas_id++) => {
    ...
    let ret = {
        ...
        g: {
            ...
            getPixelData: (x, y) => graphics.getImageData(x, y, 1, 1).data,
            getPixelColor: (x, y) => {
                let data = ret.g.getPixelData(x, y);

                //if(data[0]==0 && data[1]==0 && data[2]==0) return Color.BLACK
                //if(data[0]==255 && data[1]==0 && data[2]==0) return Color.RED
                //if(data[0]==0 && data[1]==255 && data[2]==0) return Color.GREEN
                //if(data[0]==0 && data[1]==0 && data[2]==255) return Color.BLUE
                //if(data[0]==255 && data[1]==255 && data[2]==255) return Color.WHITE

                return Color.rgb(data[0], data[1], data[2]);
            },
            getRealPixelColor: (x,y) => ret.g.getPixelColor(x + ret.center.x,y + ret.center.y)
        },
        id: p_id,
        Draw: () => { },
        resize: (w, h) => {
    ...
    ret.c.addEventListener('touchstart', () => { ret.Mouse.down = true });
    ret.c.addEventListener('touchend', () => { ret.Mouse.down = false });
    ...
    setInterval(() => {
        ...
        if (ret.showSystem) {
            ...
            g.fillLine(0, center.y, ret.width * 100, center.y);
            g.fillLine(ret.center.x, 0, center.x, ret.height * 100);
...
const createCanvas3D = () => {
    ...
    //ret.graphics.transform(1,0,0,-1,0,0);
    //ret.g.translate(0,-ret.width);
    let gameObjects = ret.gameObjects;
    ...
    setInterval(() => {
        ...
        g.clearRect(0, 0, canvas.width / 2, canvas.height);
        ...
        if (M.x > 0) {
            ...
            let cur = new Rect(M.x, M.y, 20, 3, 'white');
            cur.rotate(1);
            cur.draw();
            ...
            (new Circle(10, M.y, 5, 'red')).draw();
            (new Circle(10 - canvas.width / 2, M.y, 5, 'red')).draw();
