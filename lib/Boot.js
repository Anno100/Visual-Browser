...
/**Erstellt ein input Item
 * 
 * @param {string} name 
 * @param {string} type 
 * @param {string|number} def 
 * 
 * @returns {item:HTMLElement,label:HTMLLabelElement,input:HTMLInputElement}
 */
const $create_item = (name, type, def = '') => {
    let item_input = $create('input', name);
    item_input.type = type;
    item_input.value = def;

    let item_label = $create('label');
    item_label.innerHTML = name;

    let item = $create('div');
    item.append(item_label, item_input);

    return { item: item, label: item_label, input: item_input }
}

/**Erstellt ein Objekt mit Listenelementen */
/**
 * 
 * @param {{displayValue:string,value:string|number}[]} options 
 */
const $create_hidden_list = (name, ...options) => {
    let div = $create('div', 'list-' + name);
    for (let i = 0; i < options.length; i++) {
        const option = options[i];
        let opt = $create('option');
        opt.value = option.value;
        opt.innerHTML = option.displayValue;
        div.appendChild(opt);
...
const $copy_hidden_list_to = (name, parentElement) => {
    let div = document.getElementById('list-' + name)
    div.childNodes.forEach(e => {
        let opt = $create('option');
        opt.value = e.value;
        opt.innerHTML = e.innerHTML;
...
const $table_new_row = (table, ...values) => {
    ...
    let types = $get_header_types(table);
    ...
    values.forEach((e, i) => {
        ...
        if (types[i].split('-')[0] == 'select') {
            let select = $create('select');
            select['type'] = types[i]
            select.className = headers[i].innerHTML;
            select.readOnly = headers[i].readOnly;
            select.value = e;
            $copy_hidden_list_to('Turn', select);
            ...
            input['type'] = types[i];
            input.className = headers[i].innerHTML;
            input.readOnly = headers[i].readOnly;
            input.value = e;
...
const $table_to_map = (table) => {
    ...
    Array.from(table.getElementsByTagName('select'))
        .forEach(e => {
            if (map.has(e.className)) {
                let x = map.get(e.className);
                x.push(e.value);
...
/**@enum */
class Color {
    static rgb = (r, g, b, a = 255) => `rgba(${r},${g},${b},${a})`;
